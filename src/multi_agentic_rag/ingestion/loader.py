...
from multi_agentic_rag.utils.paths import copy_source_document, ensure_runtime_dirs, resolve_path
...
def ingest_document(
    ...
    *,
    ...
    delta_source_document: DocumentRecord | None = None
    if active_document and active_document.version != version:
        if _version_is_newer(version, active_document.version):
            ...
            delta_source_document = active_document
        else:
            status = DocumentStatus.SUPERSEDED
            superseded_by = active_document.document_id
    copied_path = copy_source_document(
        source,
        runtime_paths["documents"],
    ...
    if delta_source_document:
        old_facts = registry.list_facts(
            document_id=delta_source_document.document_id,
            status=DocumentStatus.ACTIVE,
        )
        deltas = compute_fact_deltas(
            system_name=system_name,
            from_version=delta_source_document.version,
            to_version=version,
            old_facts=old_facts,
            new_facts=facts,
        )
    ...
    if delta_source_document:
        ...
        superseded_chunks = registry.list_chunks(document_id=delta_source_document.document_id)
        superseded_facts = registry.list_facts(document_id=delta_source_document.document_id)
    if document.status == DocumentStatus.ACTIVE:
        ...
        for active in other_active_documents:
            replacement_status = (
                ...
                if active.version == document.version
                else DocumentStatus.SUPERSEDED
            )
            registry.update_document_status(
                active.document_id,
                replacement_status,
        ...
        same_version_superseded_documents = [
            candidate
            for candidate in registry.list_documents(
                system_name=system_name,
                status=DocumentStatus.SUPERSEDED,
            )
            if candidate.version == document.version and candidate.document_id != document.document_id
        ]
        for candidate in same_version_superseded_documents:
            registry.update_document_status(
                candidate.document_id,
                DocumentStatus.ARCHIVED,
                superseded_by=document.document_id,
            )
            updated = registry.get_document(candidate.document_id)
            if updated:
                superseded_documents.append(updated)
                superseded_chunks.extend(registry.list_chunks(document_id=candidate.document_id))
                superseded_facts.extend(registry.list_facts(document_id=candidate.document_id))

    warnings: list[str] = []
    _index_vectors(settings, chunks + superseded_chunks, warnings)
...
def _index_vectors(settings: Settings, chunks: list[ChunkRecord], warnings: list[str]) -> None:
    try:
        ChromaVectorStore(settings.chroma_path).index_chunks(chunks)
    except Exception as exc:  # pragma: no cover - depends on local optional dependency
        warnings.append(f"Chroma indexing skipped: {exc}")
